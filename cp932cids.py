...
def parse_CP932():
    cp932_unicodes = set()
    for line in CP932TXT.split("\n"):
        line = line.rstrip()
        if line.startswith("#"):
            continue
        cp932_code, uni, _ = line.split("\t")
        if uni.strip() == "":
            continue
        cp932_code = int(cp932_code, 16)
        uni = int(uni, 16)
        cp932_unicodes.add(uni)
    return cp932_unicodes
...
def main():
    cp932_unicodes = parse_CP932()
    cmap = CMap(sys.argv[1]).cmap()

    cp932_cids = set()
    for uni, cid in cmap.items():
        if uni in cp932_unicodes:
            cp932_cids.add(cid)
    print "cp932 ({}): {}".format(len(cp932_cids), compact_cids(sorted(cp932_cids)))
