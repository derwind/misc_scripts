...
def parse_JIS0208():
    ...
    for line in JIS0208TXT.split("\n"):
        line = line.rstrip()
        if line.startswith("#"):
            continue
        sjis_code, jisx0208code, uni, _ = line.split("\t")
        sjis_code = int(sjis_code, 16)
        jisx0208code = int(jisx0208code, 16)
        uni = int(uni, 16)
        if is_level1(jisx0208code):
            level1_unicodes.add(uni)
        elif is_level2(jisx0208code):
            level2_unicodes.add(uni)
    return level1_unicodes, level2_unicodes

def compact_cids(cids):
    ccids = []
    s = e = -1
    for cid in cids:
        if e < 0:
...
def main():
    level1_unicodes, level2_unicodes = parse_JIS0208()
    cmap = CMap(sys.argv[1]).cmap()

    level1_cids = set()
    level2_cids = set()
    for uni, cid in cmap.items():
        if uni in level1_unicodes:
            level1_cids.add(cid)
        elif uni in level2_unicodes:
            level2_cids.add(cid)
    print "level1 ({}): {}".format(len(level1_cids), compact_cids(sorted(level1_cids)))
    print "-"*20
    print "level2 ({}): {}".format(len(level2_cids), compact_cids(sorted(level2_cids)))
